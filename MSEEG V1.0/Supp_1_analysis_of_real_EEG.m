...
for m = 1:M
    ...
    load(['EEG\Patient_' num2str(m) '_Data_corrected.mat'],'clean_EEG','fs','Channel_label','EEG_mask');
    % Reshaping data
    [Ne, ch_n, N] = size(clean_EEG);
    X_eeg = zeros(ch_n, Ne*N);
    for ne = 1:Ne
        st = 1 + (ne-1)*N;
        fi = ne*N;
        X_eeg(:,st:fi) = clean_EEG(ne,:,:);
    end
    mask = [];
    for i = 1:length(EEG_mask)
        mask = [mask ones(1,N).*(EEG_mask(i)==1)];
    end
    % Plotting
    t = 0:1/fs:(N*Ne-1)/fs;
    figure('Color',[1,1,1],'Position',[0 -200 1000 850]);
    subplot(20,1,1:18)
    plot_multichannel(X_eeg, sh, fs,'b'); hold on; xlim([t(1) t(end)]);
    set(gca,'yticklabel',Channel_label,'xticklabel','','fontweight','bold','fontsize',20,'FontName','Times');
%     title('Real multi-sensor neonatal EEG','fontsize',30);
    subplot(20,1,19:20)
    ...
    if(opt == 'y' || opt == 'Y')
        print(1,['real_multi_EEG_' num2str(m)],'-dpdf','-r400');
    end
    close all;
end